...
public class EnemySpawner : MonoBehaviour
{
    ...
    private IEnumerator SpawnEnemy()
    {
        while(true)
        {
            ...
            SpawnEnemyHPSlider(clone);                      //적 체력을 나태는 Slider UI 생성 및 설정

            yield return new WaitForSeconds(spawnTime);     //spawnTimes시간 동안 대기
        }
    }   
    ...
    private void SpawnEnemyHPSlider(GameObject enemy)
    {
        ...
        //UI는 캔버스의 자식오브제트로 설정되어 있어야 화면에 보인다.
        sliderClone.transform.SetParent(canvasTransform);
        //계층 설정으로 바뀐 크기를 다시 (1,1,1)로 설정
        sliderClone.transform.localScale = Vector3.one;

        //Slider Ui가 쫓아다닌ㄹ 대상을 본인으로 설정
        sliderClone.GetComponent<SliderPositionAutoSetter>().Setup(enemy.transform);
        //Slider UI에 자신의 채력 정보를 표시하도록 설정
